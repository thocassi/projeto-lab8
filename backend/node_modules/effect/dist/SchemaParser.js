...
export function make(schema) {
  return makeConstructorSync(SchemaAST.toType(schema.ast));
}
/**
...
function makeIs(ast) {
  ...
  if (guard !== undefined) {
    return input => {
      try {
        ...
        InternalSchemaCause.getSchemaIssueOrThrow(Cause.die(error), "Type guard adapter can only return false for schema issues");
        return false;
      }
    };
  }
  ...
  return input => {
    const exit = Effect.runSyncExit(parserResult(parser(input, SchemaAST.defaultParseOptions), input));
    if (Exit.isSuccess(exit)) return true;
    InternalSchemaCause.getSchemaIssueOrThrow(exit.cause, "Type guard adapter can only return false for schema issues");
    return false;
  };
...
export function decodeUnknownSync(schema, options) {
  return CompilerRegistry.compilerAdaptersEnabled ? makeSync(schema.ast, options) : asSync(decodeUnknownEffect(schema, options));
}
/**
 * Creates a synchronous decoder for input already typed as the schema's `Encoded`
 * type.
...
export function encodeUnknownSync(schema, options) {
  return CompilerRegistry.compilerAdaptersEnabled ? makeSync(SchemaAST.flip(schema.ast), options) : asSync(encodeUnknownEffect(schema, options));
}
/**
 * Creates a synchronous encoder for input already typed as the schema's decoded
 * `Type`.
...
function makeSync(ast, options) {
  let run;
  return (input, overrideOptions) => (run ??= makeSyncEntry(CompilerRegistry.resolve(ast), options))(input, overrideOptions);
}
function makeSyncEntry(entry, options) {
  const decode = entry.decode;
  let detailed;
  const run = decode === undefined ? (input, parseOptions = SchemaAST.defaultParseOptions) => (detailed ??= makeDetailedSync(entry))(input, parseOptions) : (input, parseOptions = SchemaAST.defaultParseOptions) => {
    if (input === InternalParser.missing) {
      return (detailed ??= makeDetailedSync(entry))(input, parseOptions);
    }
    let output;
    try {
    ...
    if (output !== CompilerRegistry.invalid) return output;
    return (detailed ??= makeDetailedSync(entry))(input, parseOptions);
  };
  return options === undefined ? run : (input, overrideOptions) => run(input, mergeParseOptions(options, overrideOptions));
}
function makeDetailedSync(entry) {
  return asSync(runWithCompiler(() => entry.decodeEffect, entry.ast));
}
...
function makeConstructorSync(ast) {
  ...
  return (input, options) => {
    ...
    if (make !== undefined && input !== InternalParser.missing) {
      ...
      if (output !== CompilerRegistry.invalid && output !== InternalParser.missing) return output;
    }
    const result = (parser ??= entry.makeEffect)(input, parseOptions);
    return runSync(parserResult(result, input), "Constructor adapter can only throw schema issues");
  };
}
const normalCompiler = ast => CompilerRegistry.resolve(ast).parser;
const constructorCompiler = ast => CompilerRegistry.resolve(ast).makeEffect;
