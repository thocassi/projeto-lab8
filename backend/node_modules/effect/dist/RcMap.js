...
export const make = options => Effect.withFiber(fiber => {
  ...
  return Effect.as(Scope.addFinalizerExit(scope, () => {
    ...
    return Effect.forEach(map, ([, entry]) => Effect.exit(closeEntry(entry))).pipe(Effect.tap(() => Effect.sync(() => {
...
export const get = /*#__PURE__*/dual(2, (self, key) => Effect.uninterruptibleMask(restore => {
  ...
  if (o._tag === "Some") {
    ...
    Effect.suspend(() => self.lookup(key)).pipe(Effect.runForkWith(Context.makeUnsafe(context)), Fiber.runIn(entry.scope)).addObserver(exit => Deferred.doneUnsafe(entry.deferred, exit));
  }
...
const closeEntry = entry => entry.fiber ? Fiber.interrupt(entry.fiber).pipe(Effect.andThen(Scope.close(entry.scope, Exit.void))) : Scope.close(entry.scope, Exit.void);
