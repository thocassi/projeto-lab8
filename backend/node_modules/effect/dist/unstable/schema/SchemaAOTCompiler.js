 ...
 * @since 4.0.0
 */
import * as Codegen from "../../internal/schema/codegen.js";
import * as SchemaAST from "../../SchemaAST.js";
const helper = name => `runtime.${name}`;
const decoderOperationOrder = ["is", "decode", "make", "decodeEffect", "makeEffect"];
const operationOrder = ["decode", "is", "make"];
const compiler = sources => {
 ...
 * @since 4.0.0
 */
export const compile = targets => {
  const seen = new Map();
  const bindings = [];
  const factories = [];
  const factoryNames = new Map();
  const installations = [];
  const addSource = (node, plan, operation) => {
  ...
  const visitDependencies = (node, name, operation) => {
    switch (node._tag) {
      case "Declaration":
      ...
      case "Arrays":
        ...
        break;
      case "Objects":
        node.propertySignatures.forEach((property, index) => visit(property.type, `${name}.propertySignatures[${index}].type`, operation));
        node.indexSignatures.forEach((signature, index) => {
          visit(SchemaAST.parameterFromPropertyKey(signature.parameter), `${helper("parameterFromPropertyKey")}(${name}.indexSignatures[${index}].parameter)`, operation);
  ...
  function visit(node, reference, operation) {
  ...
  return ["// Generated by SchemaAOTCompiler. Regenerate after schema or Effect changes.", "import { runtime } from \"effect/unstable/schema/SchemaCompiler/runtime\";", ...factories, "/** @param {ReadonlyArray<import(\"effect/SchemaAST\").AST>} asts */", "export function install(asts){", ...bindings, ...installations, "}", ""].join("\n");
};
//# sourceMappingURL=SchemaAOTCompiler.js.map