...
import * as NetAddress from "../net/NetAddress.js";
...
export const layerClientProtocolHttp = options => Layer.effect(RpcClientProtocol)(Effect.gen(function* () {
  ...
  return {
    ...
    make: address => {
      const clientWithUrl = HttpClient.mapRequest(client, HttpClientRequest.prependUrl(`http${https ? "s" : ""}://${NetAddress.formatUrlHostString(address.host)}:${address.port}${path}`));
...
export const layerClientProtocolWebsocket = options => Layer.effect(RpcClientProtocol)(Effect.gen(function* () {
  ...
  return {
    ...
    make: Effect.fnUntraced(function* (address) {
      const socket = yield* Socket.makeWebSocket(`ws${https ? "s" : ""}://${NetAddress.formatUrlHostString(address.host)}:${address.port}${path}`).pipe(Effect.provideService(Socket.WebSocketConstructor, constructor));
