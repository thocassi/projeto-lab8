...
export const make = storage => Effect.sync(() => {
  ...
  return MessageStorage.of({
    ...
    unregisterShardReplyHandlers: (shardId, options) => Effect.sync(() => {
      ...
      shardSet.forEach(handler => {
        ...
        handler.resume(options?.interrupt ? ClusterAbandon.interrupt : Effect.fail(new EntityNotAssignedToRunner({
          address: handler.message.envelope.address
        })));
      });
    }),
    saveReply(reply) {
      ...
      return Effect.flatMap(storage.saveReply(reply), persisted => {
        ...
        return handlers.length === 1 ? handlers[0].respond(persisted) : Effect.forEach(handlers, handler => handler.respond(persisted));
      });
    }
  });
});
...
export const makeEncoded = /*#__PURE__*/Effect.fnUntraced(function* (encoded) {
  ...
  const storage = yield* make({
    ...
    saveReply: reply => Reply.serialize(reply, codecForJson).pipe(Effect.map(encodedReply => ({
      encodedReply,
      persisted: reply
      ...
      return Effect.map(Effect.orDie(Reply.serialize(persisted, codecForJson)), encodedReply => ({
        encodedReply,
        persisted
      }));
    }), Effect.flatMap(({
...
export class MemoryDriver extends /*#__PURE__*/Context.Service()("effect/cluster/MessageStorage/MemoryDriver", {
  make: /*#__PURE__*/Effect.gen(function* () {
    ...
    const encoded = {
      ...
      resetAddresses,
      resetRequests: requestIds => Effect.sync(() => {
