...
/** @internal */
export function getParamNames(schema) {
  if (schema === undefined) return emptyParamNames;
  const ast = SchemaAST.getLastEncoding(schema.ast);
  return SchemaAST.isObjects(ast) && ast.indexSignatures.length === 0 ? new Set(ast.propertySignatures.map(ps => String(ps.name))) : undefined;
}
/** @internal */
export function toRouterPath(path, schema) {
  ...
  for (let i = 0; i < path.length; i++) {
    ...
    if (depth > 0 || i === regexStart) {
      out += char;
      if (char === "\\") out += path[++i] ?? "";else if (char === "(") depth++;else if (char === ")") depth--;
      continue;
    }
    if (char !== ":") {
      out += char;
