...
import * as HttpApiPath from "./internal/path.js";
...
export const layer = (api, options) => HttpRouter.use(Effect.fnUntraced(function* (router) {
  ...
  if (options?.openapiPath) {
    const makeResponse = Function.memoize(api => Response.jsonUnsafe(OpenApi.fromApi(api)));
    yield* router.add("GET", options.openapiPath, Effect.sync(() => makeResponse(api)));
  }
}));
/**
...
export const group = (api, groupIdentifier, build) => Layer.effectContext(Effect.gen(function* () {
  ...
  for (const item of handlers.handlers.values()) {
    routes.push(handlerToRoute(api, group, item, services));
  }
 ...
 * @since 4.0.0
 */
export const handler = (_api, _groupIdentifier, _endpointIdentifier, f) => f;
...
export const endpoint = (api, groupIdentifier, endpointIdentifier, handler) => Effect.contextWith(context => {
  ...
  return Effect.succeed(handlerToHttpEffect(api, group, endpoint, Context.omit(Scope.Scope)(context), handler, false));
});
/**
...
function buildPayloadDecoders(payloadMap, options) {
...
function handlerToHttpEffect(api, group, endpoint, context, handler, isRaw) {
  ...
  const decodeUnknownEffect = schema => Schema.decodeUnknownEffect(schema, options);
  ...
  const decodeQuery = UndefinedOr.map(endpoint.query, schema => decodeUnknownEffect(Schema.toCodecArrayFromSingle(schema)));
...
export function handlerToRoute(api, group, handler, context) {
  ...
  return HttpRouter.route(endpoint.method, HttpApiPath.toRouterPath(endpoint.path, endpoint.params), handlerToHttpEffect(api, group, endpoint, context, handler.handler, handler.isRaw), {
...
function makeWithHeadersEncoder(endpoint, options) {
...
function makeStreamEncoder(endpoint, options) {
...
function makeSseEncoder(streamSchema, options) {
  const CauseSchema = Schema.toCodecJson(Schema.Cause(streamSchema.error, Schema.Defect()));
  return {
    sseMode: streamSchema.sseMode,
    encodeEvents: Schema.encodeUnknownEffect(Schema.Array(streamSchema.events), options),
...
function encodeSseStream(stream, encoder) {
  return stream.pipe(encoder.sseMode === "data" ? Stream.map(value => ({
    ...
    event: "message",
    data: value
  })) : Function.identity, Stream.mapArrayEffect(chunk => Effect.orDie(encoder.encodeEvents(chunk))), Stream.catchCause(cause => Stream.fromEffect(encodeFailureEvent(cause, encoder))), Stream.map(renderSseEvent), Stream.encodeText);
}
