...
function makeOpenApi(api, options) {
  let spec = {
    openapi: "3.1.0",
    info: {
      title: "Api",
  ...
  HttpApi.reflect(api, {
    ...
    onEndpoint({
      ...
      const path = endpoint.path.replace(/:(\w+)\??/g, (match, key) => paramNames === undefined || paramNames.has(key) ? `{${key}}` : match);
      const method = endpoint.method.toLowerCase();
      const isQuery = method === "query";
      const operationKey = isQuery ? "QUERY" : method;
      const operationPath = isQuery ? ["paths", path, "x-oai-additionalOperations", operationKey] : ["paths", path, operationKey];
            ...
            streamContent.forEach((stream, contentType) => {
              ...
              if (HttpApiSchema.isStreamSse(stream)) {
                ...
                pathOps.push({
                  ...
                  path: [...operationPath, "responses", String(status), "content", contentType, "x-effect-stream", "causeSchema"]
                });
                pathOps.push({
                  _tag: "schema",
                  ast: SchemaAST.getAST(stream.error),
                  path: [...operationPath, "responses", String(status), "content", contentType, "x-effect-stream", "errorSchema"]
                });
                InternalRecord.assignProperty(op.responses[status].content, contentType, {
                  schema: {},
                  "x-effect-stream": {
  ...
  if (Arr.isArrayNonEmpty(pathOps)) {
    const jsonSchemaMultiDocument = JsonSchema.toMultiDocumentOpenApi3_1(InternalToJsonSchemaDocument.toJsonSchemaMultiDocument(InternalToRepresentation.toRepresentations(Arr.map(pathOps, op => InternalToCodec.toCodecJsonAST(op.ast)), options), {
