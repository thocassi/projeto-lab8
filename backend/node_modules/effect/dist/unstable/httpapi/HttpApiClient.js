  ...
  HttpApi.reflect(api, {
    ...
    onEndpoint(onEndpointOptions) {
      ...
      for (const streamSuccess of getStreamSuccessSchemas(endpoint)) {
        ...
        addResponseAlternative(successAlternatives, HttpApiSchema.getStatusSuccessSchema(streamSuccess), streamSchema.contentType, streamToResponse(streamSuccess, parseOptions));
      }
      for (const [status, alternatives] of successAlternatives.entries()) {
        decodeMap[status] = makeResponseDecoder(alternatives);
      }
      const encodeUnknownEffect = schema => Schema.encodeUnknownEffect(schema, parseOptions);
      ...
      const encodePayload = Arr.isArrayNonEmpty(payloadSchemas) ? HttpMethod.hasBody(endpoint.method) ? encodeUnknownEffect(getEncodePayloadSchema(payloadSchemas, endpoint.method)) : encodeUnknownEffect(Schema.Union(payloadSchemas)) : undefined;
...
export const urlBuilder = (api, options) => {
  ...
  HttpApi.reflect(api, {
    ...
    onEndpoint({
      ...
      const parseOptions = Context.getOrUndefined(mergedAnnotations, HttpApi.ParseOptions);
      const makeUrl = compilePath(endpoint.path, endpoint.params);
      const encodeParams = endpoint.params === undefined ? undefined : Schema.encodeSync(endpoint.params, parseOptions);
      const encodeQuery = endpoint.query === undefined ? undefined : Schema.encodeSync(endpoint.query, parseOptions);
...
function schemasToResponse(schemas, options) {
...
function streamToResponse(successSchema, options) {
...
function makeSseDecoder(declaration, parseOptions) {
  ...
  return options => options === undefined ? defaultDecoder : Sse.decodeSchema(Event, options, parseOptions);
}
