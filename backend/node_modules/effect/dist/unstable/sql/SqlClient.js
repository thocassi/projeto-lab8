...
export const make = /*#__PURE__*/Effect.fnUntraced(function* (options) {
  ...
  const control = options.prepareTransactionControls === true ? (conn, sql) => conn.execute(sql, [], undefined) : (conn, sql) => conn.executeUnprepared(sql, [], undefined);
  const withTransaction = makeWithTransaction({
    ...
    releaseSavepoint: releaseSavepoint ? (conn, id) => control(conn, releaseSavepoint(`effect_sql_${id}`)) : undefined,
  ...
  const borrower = options.borrower === undefined ? undefined : f => Effect.flatMap(Effect.serviceOption(transactionService), Option.match({
    onNone: () => options.borrower(f),
    onSome: ([conn]) => f(conn)
  }));
  const client = Object.assign(Statement.make(getConnection, options.compiler, options.spanAttributes, options.transformRows, borrower), {
    [TypeId]: TypeId,
    safe: undefined,
    withTransaction,
    transactionService,
    ...
    withoutTransforms() {
      ...
      const statement = Statement.make(getConnection, options.compiler.withoutTransform, options.spanAttributes, undefined, borrower);
