 ...
 * @since 4.0.0
 */
export const SpanPropagationEnabled = /*#__PURE__*/Context.Reference("effect/sql/SpanPropagationEnabled", {
  defaultValue: () => false
});
/**
 ...
 * @since 4.0.0
 */
export const make = (acquirer, compiler, spanAttributes, transformRows, borrower) => {
  ...
  const self = Object.assign(function sql(strings, ...args) {
    if (typeof strings === "string") {
      ...
      return statement(acquirer, compiler, strings, args, spanAttributes, transformRows, borrower);
    }
    throw "absurd";
  }, {
    unsafe(sql, params) {
      return makeUnsafe([literal(sql, params)], acquirer, compiler, spanAttributes, transformRows, borrower);
    },
    literal(sql) {
 ...
 * @since 4.0.0
 */
export const statement = (acquirer, compiler, strings, args, spanAttributes, transformRows, borrower) => {
...
const makeUnsafe = (segments, acquirer, compiler, spanAttributes, transformRows, borrower) => {
...
const StatementProto = {
  ...
  withConnectionSpan(operation, f, withoutTransform, span) {
    return withStatement(this, span, (statement, fiber) => {
      ...
      const execute = this.borrower === undefined ? Effect.scoped(Effect.flatMap(this.acquirer, _ => f(_, sql, params))) : this.borrower(connection => f(connection, sql, params));
      return fiber.cache.tracerEnabled && fiber.getRef(SpanPropagationEnabled) ? Effect.provideService(execute, Tracer.ParentSpan, span) : execute;
    });
  },
  get withoutTransform() {
  ...
  get stream() {
    ...
    return Stream.unwrap(Effect.flatMap(Effect.makeSpanScoped("sql.execute", {
      kind: "client"
    }), span => withStatement(self, span, (statement, fiber) => {
      ...
      return fiber.cache.tracerEnabled && fiber.getRef(SpanPropagationEnabled) ? Effect.succeed(Stream.provideService(Stream.unwrap(acquire), Tracer.ParentSpan, span)) : acquire;
    })));
  },
  get values() {
...
const withStatement = (self, span, f) => Effect.withFiber(fiber => {
  ...
  return Effect.flatMap(transform(self, make(self.acquirer, self.compiler, self.spanAttributes, self.transformRows, self.borrower), fiber, span), statement => f(statement, fiber));
});
const isSegment = u => {
