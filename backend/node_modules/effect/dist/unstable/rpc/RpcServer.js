...
export const makeNoSerialization = /*#__PURE__*/Effect.fnUntraced(function* (group, options) {
  ...
  const write = (clientId, message, opts) => Effect.catchDefect(Effect.withFiber(requestFiber => {
    ...
    } else if (client.ended && (message._tag !== "Interrupt" || !client.fibers.has(message.requestId))) {
  ...
  const handleRequest = (requestFiber, client, request, opts) => {
    ...
    const runFork = Effect.runForkWith(Context.add(handlerContext(entry, requestFiber.context), Scope.Scope, scope));
...
export const make = /*#__PURE__*/Effect.fnUntraced(function* (group, options) {
  ...
  const handleEncode = (client, requestId, schemas, effect, tag) => {
    ...
    const write = Exit.isExit(effect) && Exit.isSuccess(effect) ? send(client.id, responseEnvelope(requestId, tag, effect.value), collector && collector.clearUnsafe()) : Effect.flatMap(Effect.provideContext(collector ? Effect.provideService(effect, Transferable.Collector, collector) : effect, schemas.context), value => send(client.id, responseEnvelope(requestId, tag, value), collector && collector.clearUnsafe()));
    return Effect.catchCause(write, cause => {
      ...
      return Effect.andThen(sendRequestDefect(client, requestId, schemas.encodeDefect, defect), server.write(client.id, {
        _tag: "Interrupt",
        requestId,
        interruptors: []
      }));
  ...
  const writeDecodedRequest = (client, requestId, schemas, request, payload) => {
    ...
    decoded.headers = Headers.fromInput(request.headers);
...
export const makeProtocolWithHttpEffect = /*#__PURE__*/Effect.fnUntraced(function* (options = {}) {
  ...
  const httpEffect = Effect.gen(function* () {
    ...
    yield* Scope.addFinalizerExit(scope, () => {
      ...
      return Effect.andThen(Queue.shutdown(queue), Effect.forEach(requestIds, requestId => writeRequest(id, {
        _tag: "Interrupt",
        requestId
      }), {
        discard: true
...
export const makeProtocolStdio = /*#__PURE__*/Effect.gen(function* () {
  ...
  return yield* Protocol.make(Effect.fnUntraced(function* (writeRequest) {
    const queue = yield* Queue.bounded(8);
    const parser = serialization.makeUnsafe();
...
const makeSocketProtocol = /*#__PURE__*/Effect.gen(function* () {
  ...
  const clientIds = new Set();
  let writeRequest;
  const onSocket = Effect.fnUntraced(function* (socket, headers) {
    ...
    const write = response => {
      try {
        ...
        return Effect.orDie(writer.write(parser.encode(ResponseDefectEncoded(encodeDefectUnsafe(cause)))));
      }
    };
    clients.set(id, {
      write
