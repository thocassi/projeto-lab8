 ...
 * @since 4.0.0
 */
export const LanguageModel = /*#__PURE__*/Context.Service("effect/unstable/ai/LanguageModel");
/**
 ...
 * @since 4.0.0
 */
export const TypeId = "~effect/ai/LanguageModel";
/**
...
export const make = /*#__PURE__*/Effect.fnUntraced(function* (params) {
  ...
  const generateContent = Effect.fnUntraced(function* (options, providerOptions) {
    ...
    if (hasPendingApprovals) {
      ...
      if (preResolvedResults.length > 0) {
        providerOptions.prompt = Prompt.fromMessages([...providerOptions.prompt.content, ...Prompt.fromResponseParts(preResolvedResults).content]);
      }
    }
    ...
    const ResponseSchema = Schema.mutable(Schema.Array(Response.Part(options.disableToolCallResolution === true ? makeToolkitWithEncodedParameters(toolkit) : makeToolkitWithOpaqueParameters(toolkit))));
    ...
    if (options.disableToolCallResolution === true) {
      ...
      return [...preResolvedResults, ...content];
    }
    const rawContent = yield* generateWithNonIncrementalFallback();
    ...
    const toolResults = incompleteFinishReason !== undefined ? rawContent.filter(part => part.type === "tool-call" && part.providerExecuted !== true && toolkit.tools[part.name] !== undefined).map(part => makeInterruptedToolResult(part, incompleteFinishReason)) : yield* resolveToolCalls(rawContent, toolkit, providerOptions.prompt.content, concurrency).pipe(Stream.filter(result => result.type === "tool-approval-request" || result.preliminary === false), Stream.runCollect);
    if (tracker) {
      const responseMetadata = content.find(part => part.type === "response-metadata");
      if (Predicate.isNotUndefined(responseMetadata) && Predicate.isNotUndefined(responseMetadata.id)) {
        tracker.markParts(providerOptions.prompt.content, responseMetadata.id);
  ...
  const streamContent = Effect.fnUntraced(function* (options, providerOptions) {
    ...
    const ResponseSchema = Schema.NonEmptyArray(Response.StreamPart(options.disableToolCallResolution === true ? makeToolkitWithEncodedParameters(toolkit) : makeToolkitWithOpaqueParameters(toolkit)));
...
const createDenialResults = denials => {
  ...
  for (const denial of denials) {
    if (Predicate.isNotUndefined(denial.toolCall)) {
      results.push(Response.makePart("tool-result", {
...
const makeToolkitWithOpaqueParameters = toolkit => Toolkit.make(...Object.values(toolkit.tools).map(tool => tool.setParameters(Schema.Unknown)));
// Provider-executed tools bypass Toolkit, so validate their parameters here.
const validateProviderExecutedToolCalls = (toolkit, parts) => Effect.forEach(parts, part => {
  if (part.type !== "tool-call" || part.providerExecuted !== true) {
    return Effect.void;
  }
  const tool = toolkit.tools[part.name];
  ...
  return Effect.asVoid(Schema.decodeUnknownEffect(tool.parametersSchema)(part.params));
}, {
  discard: true
});
