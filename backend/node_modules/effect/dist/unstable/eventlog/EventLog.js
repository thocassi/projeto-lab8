...
export const groupCompaction = (group, effect) => Layer.effectDiscard(Effect.gen(function* () {
  ...
  yield* registry.registerCompaction({
    ...
    effect: Effect.fnUntraced(function* ({
      ...
      const decodePayload = (tag, payload) => Schema.decodeUnknownEffect(group.events[tag].payloadSchemaBinary)(payload).pipe(Effect.updateContext(input => Context.merge(services, input)), Effect.orDie);
      const writePayload = Effect.fnUntraced(function* (timestamp, tag, payload) {
        ...
        const entry = new Entry({
          ...
          event: tag,
          payload: yield* Schema.encodeUnknownEffect(event.payloadSchemaBinary)(payload).pipe(Effect.orDie),
...
const make = /*#__PURE__*/Effect.gen(function* () {
  ...
  const writeHandler = Effect.fnUntraced(function* (handler, options) {
    const payload = yield* Schema.encodeUnknownEffect(handler.event.payloadSchemaBinary)(options.payload).pipe(Effect.orDie);
