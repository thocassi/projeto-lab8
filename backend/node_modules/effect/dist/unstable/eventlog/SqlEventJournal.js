...
export const make = options => Effect.gen(function* () {
  ...
  const writeFromRemote = Effect.fnUntraced(function* (options) {
    ...
    if (entries.length > 0) {
      yield* sql`SELECT id FROM ${entryTableSql} WHERE ${sql.in("id", entries.map(entry => entry.id))}`.pipe(Effect.flatMap(decodeEntryIdRows), Effect.tap(rows => Effect.sync(() => {
  ...
  return EventJournal.EventJournal.of({
    ...
    write: Effect.fnUntraced(function* ({
      ...
      return yield* Effect.uninterruptibleMask(restore => restore(effect(entry)).pipe(Effect.tap(insertEntry(toEntryRow(entry)).pipe(Effect.mapError(cause => new EventJournal.EventJournalError({
        cause,
        method: "write"
    ...
    withRemoteUncommited: Effect.fnUntraced(function* (remoteId, f) {
      const entries = yield* sql`
            ...
            ORDER BY timestamp ASC
          `.pipe(Effect.flatMap(decodeEntryRows), Effect.map(toEntries), Effect.mapError(cause => new EventJournal.EventJournalError({
        cause,
...
const SqlUint8Array = /*#__PURE__*/Schema.Union([Schema.Uint8Array, ArrayBuffer]).pipe(/*#__PURE__*/Schema.decodeTo(Schema.Uint8Array, /*#__PURE__*/SchemaTransformation.transform({
  decode: value => value instanceof globalThis.ArrayBuffer ? new globalThis.Uint8Array(value) : value,
  encode: value => value
})));
const SqlEntryId = /*#__PURE__*/SqlUint8Array.pipe(/*#__PURE__*/Schema.decodeTo(EventJournal.EntryId));
...
const EntryIdRowArray = /*#__PURE__*/Schema.Array(/*#__PURE__*/Schema.Struct({
  id: SqlEntryId
}));
