...
const codePointIntervals = [{
  minimum: 0,
  maximum: maximumCodePoint
}];
const printableAsciiIntervals = [{
  minimum: 32,
  maximum: 126
}];
const preferredCodePoints = [... /*#__PURE__*/globalThis.Array.from({
  length: 95
}, (_, index) => index + 32), 9, 10, 11, 12, 13, 0, 0x80, 0xe9, 0x3a9, 0x1f600];
function characterMetadata(intervals, length) {
  return {
    preferred: preferredCodePoints.filter(codePoint => codePointLength(codePoint) === length && codePointInIntervals(codePoint, intervals)),
    count: intervals.reduce((count, interval) => count + countCodePoints(interval, length), 0)
  };
}
function character(intervals) {
...
function alternation(nodes) {
  const flattened = [];
  for (const node of nodes) {
    if (node._tag === "Alternation") flattened.push(...node.nodes);else flattened.push(node);
  }
...
function restrictIntervals(intervals, maximum) {
  return intervals.flatMap(interval => interval.minimum > maximum ? [] : [{
    minimum: interval.minimum,
    maximum: Math.min(interval.maximum, maximum)
  }]);
}
function restrictCharacterCodePoints(node, maximum) {
  switch (node._tag) {
    case "Character":
    ...
    case "Repetition":
      return {
        ...node,
        node: restrictCharacterCodePoints(node.node, maximum)
      };
    case "Literal":
      return node;
  }
...
function complementIntervals(intervals) {
  ...
  for (const interval of normalized) {
    if (minimum < interval.minimum) out.push({
      minimum,
      maximum: interval.minimum - 1
    });
    minimum = Math.max(minimum, interval.maximum + 1);
  }
  if (minimum <= maximumCodePoint) out.push({
    minimum,
    maximum: maximumCodePoint
  });
  return out;
}
function codePointInIntervals(codePoint, intervals) {
...
function countCodePoints(interval, length) {
  ...
  if (length === 1 && minimum <= lowSurrogateMaximum && maximum >= highSurrogateMinimum) {
    return maximum - minimum + 1 - (Math.min(maximum, lowSurrogateMaximum) - Math.max(minimum, highSurrogateMinimum) + 1);
  }
  return maximum - minimum + 1;
}
function codePointAtOffset(interval, length, offset) {
...
function generateCharacter(node, length, state) {
...
function canonicalCharacter(node, length) {
...
class Parser {
  ...
  parseCharacterClass() {
    ...
    while (this.index < this.source.length && (this.peek() !== "]" || first)) {
      ...
      if (this.peek() === "-" && this.source[this.index + 1] !== "]") {
        ...
        if (right === undefined || left.length !== 1 || right.length !== 1 || left[0].minimum !== left[0].maximum || right[0].minimum !== right[0].maximum || left[0].minimum > right[0].minimum) return undefined;
        intervals.push({
          minimum: left[0].minimum,
          maximum: right[0].minimum
        });
      } else {
        intervals.push(...left);
      }
  ...
  parseClassAtom() {
    ...
    if (token === "\\") {
      ...
      return escaped?._tag === "Character" ? escaped.intervals : escaped?._tag === "Literal" ? this.literalIntervals(escaped.value) : undefined;
    }
    ...
    switch (token) {
      ...
      case "u":
        if (this.peek() === "{") {
          ...
          this.index++;
          return value <= maximumCodePoint && !isSurrogate(value) ? literal(globalThis.String.fromCodePoint(value)) : undefined;
        }
...
function emptyLengths(limit) {
  return globalThis.Array(limit + 1).fill(false);
}
function concatenateLengths(left, right, limit) {
...
function repetitionLengths(node, child, limit) {
  const out = emptyLengths(limit);
...
function possibleLengths(node, limit, cache) {
...
function sequenceSuffixLengths(nodes, length, cache) {
  const out = new Array(nodes.length + 1);
  const terminal = emptyLengths(length);
  terminal[0] = true;
  out[nodes.length] = terminal;
  for (let index = nodes.length - 1; index >= 0; index--) {
    out[index] = concatenateLengths(possibleLengths(nodes[index], length, cache), out[index + 1], length);
  }
  return out;
}
function generateSequence(nodes, length, state, cache) {
...
function repetitionCounts(node, length, cache) {
...
function generateExact(node, length, state, cache) {
  switch (node._tag) {
    case "Literal":
    ...
    case "Alternation":
      {
        ...
        return candidates.length === 0 ? undefined : generateExact(candidates[Model.randomIndex(state, candidates.length)], length, state, cache);
      }
    case "Concatenation":
...
function canonicalSequence(nodes, length, cache) {
  const suffixLengths = sequenceSuffixLengths(nodes, length, cache);
  let remaining = length;
  let out = "";
  for (let index = 0; index < nodes.length; index++) {
...
function canonicalExact(node, length, cache) {
  switch (node._tag) {
    case "Literal":
      return node.value.length === length ? node.value : undefined;
    case "Character":
    ...
    case "Repetition":
      {
        ...
        return count === undefined ? undefined : canonicalSequence(globalThis.Array(count).fill(node.node), length, cache);
      }
  }
}
function structuralShrinks(node, regExp, value, minimumLength, cache) {
...
function test(regExp, value) {
  regExp.lastIndex = 0;
  return regExp.test(value);
}
function hasTrailingAnchor(source) {
  if (!source.endsWith("$")) return false;
  let backslashes = 0;
  for (let index = source.length - 2; index >= 0 && source[index] === "\\"; index--) backslashes++;
  return backslashes % 2 === 0;
}
function addUnanchoredSurroundings(node, source, flags) {
  const nodes = [];
  const anyCharacters = {
    _tag: "Repetition",
