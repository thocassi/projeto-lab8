...
const semanticMagnitudes = [bigint1, bigint2, /*#__PURE__*/BigInt(0x7fff_ffff), /*#__PURE__*/BigInt(0x8000_0000), /*#__PURE__*/BigInt(Number.MAX_SAFE_INTEGER), /*#__PURE__*/BigInt(Number.MAX_SAFE_INTEGER) + bigint1, /*#__PURE__*/BigInt(Number.MAX_VALUE), /*#__PURE__*/BigInt(Number.MAX_VALUE) + bigint1, bigint1 << /*#__PURE__*/BigInt(1_024)];
...
function randomMagnitude(state, maximum) {
...
function makeGenerator(minimum, maximum) {
  if (minimum !== undefined && maximum !== undefined) return Model.makeRandomNumericBigInt(minimum, maximum);
  const special = [bigint0, ...(minimum === undefined ? [] : [minimum, minimum + bigint1]), ...(maximum === undefined ? [] : [maximum, maximum - bigint1]), ...semanticMagnitudes.flatMap(value => [value, -value])].filter(value => isWithin(value, minimum, maximum));
...
function sample(value, target, context) {
  return Model.makeSampleWithLazyShrinks(value, () => {
    ...
    const candidates = new Set(context === undefined ? [target] : []);
    ...
    if (context === undefined) {
      ...
      if (Math.abs(failingBits - passingBits) > 1 && (context === bigint0 || context > bigint0 === value > bigint0)) {
...
/** @internal */
export function make(minimum, maximum) {
  const generate = makeGenerator(minimum, maximum);
  const target = minimum !== undefined && minimum > bigint0 ? minimum : maximum !== undefined && maximum < bigint0 ? maximum : bigint0;
