...
function isAllowed(value, bounds) {
...
function addScale(scale, offset, maximum = Number.MAX_SAFE_INTEGER) {
  return Math.min(maximum, scale + offset);
}
function boundaryCandidate(value, exclusive, direction) {
  if (value === undefined || exclusive !== true) return value;
  return value.scale < Number.MAX_SAFE_INTEGER ? BigDecimal.make(value.value * bigint10 + BigInt(direction), value.scale + 1) : BigDecimal.make(value.value + BigInt(direction), value.scale);
}
function digits(value) {
  return (value < bigint0 ? -value : value).toString().length;
}
function roundAtScale(value, scale, ceil, maximumPower = maximumExactPower) {
...
function coefficientRange(scale, bounds) {
  const lower = bounds.minimum;
  const upper = bounds.maximum;
  let minimum = lower === undefined ? undefined : roundAtScale(lower, scale, bounds.exclusiveMinimum !== true);
  let maximum = upper === undefined ? undefined : roundAtScale(upper, scale, bounds.exclusiveMaximum === true);
  ...
  if (lower !== undefined && minimum === undefined && maximum !== undefined) {
    minimum = roundAtScale(lower, scale, bounds.exclusiveMinimum !== true, Math.max(maximumExactPower, digits(maximum)));
  }
  if (upper !== undefined && maximum === undefined && minimum !== undefined) {
    maximum = roundAtScale(upper, scale, bounds.exclusiveMaximum === true, Math.max(maximumExactPower, digits(minimum)));
  }
...
function randomCoefficient(state, range) {
...
function randomScale(state, boundaryScale) {
...
function projectExact(value, scale) {
  const difference = BigInt(scale) - BigInt(value.scale);
  if (difference < bigint0 || difference > BigInt(maximumExactPower)) return undefined;
  return value.value * bigint10 ** difference;
}
function midpoint(passing, failing, maximumScale) {
...
function sample(value, target, context, maximumScale = addScale(Math.max(value.scale, target.scale), 128)) {
  return Model.makeSampleWithLazyShrinks(value, () => {
    ...
    const candidates = (context === undefined ? shrinkExponents.map(exponent => BigDecimal.make(BigInt(direction), -exponent)) : [next]).filter(candidate => candidate !== undefined && candidate.scale <= maximumScale && compare(candidate, from) === direction && compare(candidate, value) === -direction);
    ...
    return Effect.suspend(() => {
      ...
      const candidate = index < candidates.length ? candidates[index++] : previous === undefined ? undefined : midpoint(previous[0], value, maximumScale);
...
export function make(bounds) {
  ...
  const special = [witness, BigDecimal.make(bigint1, 1), BigDecimal.make(bigint1, 0), BigDecimal.make(bigint10, 0), BigDecimal.make(-bigint1, 1), BigDecimal.make(-bigint1, 0), BigDecimal.make(-bigint10, 0), ...(bounds.minimum !== undefined && bounds.exclusiveMinimum !== true ? [bounds.minimum] : []), ...(bounds.maximum !== undefined && bounds.exclusiveMaximum !== true ? [bounds.maximum] : [])].filter(value => isAllowed(value, bounds));
  const boundaryScale = lower === undefined ? upper?.scale : upper === undefined ? lower.scale : Math.max(lower.scale, upper.scale);
  const preferred = boundaryScale === undefined ? undefined : coefficientRange(boundaryScale, bounds);
  const generatePreferred = preferred?.[0] !== undefined && preferred[1] !== undefined ? Model.makeRandomNumericBigInt(preferred[0], preferred[1]) : undefined;
