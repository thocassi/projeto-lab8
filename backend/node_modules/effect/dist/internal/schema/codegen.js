...
const getEncodingChecks = ast => "encodingChecks" in ast ? ast.encodingChecks : undefined;
const getExpectedKeys = ast => ast.propertySignatures.map(p => typeof p.name === "number" ? String(p.name) : p.name);
const isOptional = ast => ast.context?.isOptional ?? false;
const maxGeneratedDepth = 256;
/** @internal */
export const maxGeneratedNodes = 2048;
const failureExpression = operation => operation === "decode" ? "I" : "false";
/** @internal */
const getEmission = (ast, depth = 0, local = false, budget = {
  remaining: maxGeneratedNodes
}) => {
...
const canEmit = (ast, depth = 0) => getEmission(ast, depth) !== "unsupported";
const isMakeSafe = (ast, depth = 0, budget = {
  remaining: maxGeneratedNodes
}) => {
  if (--budget.remaining < 0 || depth > maxGeneratedDepth || ast.encoding !== undefined || ast.context?.constructorDefault !== undefined || SchemaAST.getConstructorDescriptor(ast) !== undefined) {
    return false;
  }
  switch (ast._tag) {
    case "Null":
    ...
    case "Arrays":
      return ast.elements.every(element => isMakeSafe(element, depth + 1, budget)) && ast.rest.every(element => isMakeSafe(element, depth + 1, budget));
    case "Objects":
      return ast.indexSignatures.length === 0 && ast.propertySignatures.every(property => isMakeSafe(property.type, depth + 1, budget));
    case "Union":
    case "Declaration":
    case "Suspend":
      return false;
...
const shouldCompileMake = ast => (ast._tag === "Arrays" && ast.elements.length === 0 && ast.rest.length === 1 || ast._tag === "Objects" && ast.propertySignatures.length > 0 && ast.indexSignatures.length === 0) && isMakeSafe(ast);
const variable = emitter => `v${emitter.next++}`;
const propertyKey = (emitter, key, reference) => typeof key === "string" ? JSON.stringify(key) : constant(emitter, key, reference);
const propertyPresence = (input, key, name) => name === "__proto__" ? `Object.hasOwn(${input},${key})` : `${key} in ${input}`;
const assignProperty = (output, key, value, name) => name === "__proto__" ? `Object.defineProperty(${output},${key},{value:${value},writable:true,enumerable:true,configurable:true})` : `${output}[${key}]=${value}`;
const constant = (emitter, value, reference) => {
  const cached = emitter.constantIndexes.get(value);
  if (cached !== undefined) return `C[${cached}]`;
  const index = emitter.bindings.length;
  emitter.bindings.push({
    value,
    reference
  });
...
const propertyNeedsPresenceCheck = (name, ast) => name === "__proto__" || needsPresenceCheck(ast);
/** @internal */
export const shouldCompileParser = (ast, local = false) => {
...
const lookupMemberReferences = (ast, path) => {
  switch (ast._tag) {
    case "Null":
      return ["null"];
    case "Undefined":
...
function emit(ast, input, statements, emitter, operation, path) {
  ...
  const astConstant = ast.checks !== undefined || encodingChecks !== undefined ? constant(emitter, ast, path) : undefined;
  ...
  statements.push(`const ${checked}=${output}`, `if(K(${astConstant},${checked},0,o))return ${invalid}`);
  return operation === "decode" ? checked : "true";
}
const emitDecoderHelper = (ast, emitter, operation, path) => {
  ...
  emitter.helpers.push(`function ${name}(i,o){${statements.join(";")};return ${output}}`);
  return name;
};
const emitUnionHelper = (ast, emitter, operation, path) => {
  const cached = emitter.unionHelpers.get(ast);
  if (cached !== undefined) return cached;
  const name = `u${emitter.next++}`;
  emitter.unionHelpers.set(ast, name);
  const entries = ast.types.map((type, index) => `[${constant(emitter, type, `${path}.types[${index}]`)},${emitDecoderHelper(type, emitter, operation, `${path}.types[${index}]`)}]`);
  emitter.initializers.push(`const ${name}=new Map([${entries.join(",")}])`);
  return name;
};
const emitIndexes = (ast, input, output, statements, emitter, operation, path) => {
  const fixedKeys = output === undefined || ast.propertySignatures.length === 0 ? undefined : constant(emitter, new Set(getExpectedKeys(ast)), `new Set(${runtimeReference("getExpectedKeys")}(${path}))`);
  for (let signatureIndex = 0; signatureIndex < ast.indexSignatures.length; signatureIndex++) {
    ...
    statements.push(`const ${keys}=${parameter._tag === "String" && parameter.checks === undefined ? `Object.keys(${input})` : `G(${input},${constant(emitter, parameter, `${signaturePath}.parameter`)},o)`}`);
    const loop = [`const ${key}=${keys}[${index}]`];
    const decodedKey = parameter._tag === "String" && parameter.checks === undefined && parameter.encoding === undefined ? key : emit(SchemaAST.parameterFromPropertyKey(parameter), key, loop, emitter, operation, `${runtimeReference("parameterFromPropertyKey")}(${signaturePath}.parameter)`);
    ...
    if (output !== undefined) {
      const assign = `if(${decodedKey}==="__proto__")Object.defineProperty(${output},${decodedKey},{value:${decoded},writable:true,enumerable:true,configurable:true});else ${output}[${decodedKey}]=${decoded}`;
      loop.push(fixedKeys === undefined ? assign : `if(!${fixedKeys}.has(${key})&&!${fixedKeys}.has(${decodedKey})){${assign}}`);
    }
    statements.push(`for(let ${index}=0;${index}<${keys}.length;${index}++){${loop.join(";")}}`);
  }
};
const emitBase = (ast, input, statements, emitter, operation, path) => {
  ...
  switch (ast._tag) {
    ...
    case "Unknown":
      return input;
    case "ObjectKeyword":
      statements.push(`if((${input}===null||typeof ${input}!=="object")&&typeof ${input}!=="function")return ${invalid}`);
      return input;
    case "Enum":
      {
        const values = constant(emitter, new Set(ast.enums.map(entry => entry[1])), `new Set(${path}.enums.map(entry=>entry[1]))`);
    ...
    case "Arrays":
      {
        ...
        if (ast.rest.length === 0) {
          statements.push(minimumElementLength === elementLength ? `if(${length}!==${elementLength})return ${invalid}` : `if(${length}<${minimumElementLength}||${length}>${elementLength})return ${invalid}`);
          ...
          for (let index = 0; index < elementLength; index++) {
            const value = variable(emitter);
            const elementStatements = [`const ${value}=${input}[${index}]`];
            const decoded = emit(ast.elements[index], value, elementStatements, emitter, operation, `${path}.elements[${index}]`);
            if (output !== undefined) elementStatements.push(`${output}[${index}]=${decoded}`);
            statements.push(index < minimumElementLength ? elementStatements.join(";") : `if(${index}<${length}){${elementStatements.join(";")}}`);
          }
          return output ?? input;
        }
        ...
        statements.push(`for(let ${index}=${elementLength};${index}<${length}-${tailLength};${index}++){${restStatements.join(";")}}`);
    ...
    case "Objects":
      {
        ...
        statements.push(`if(typeof ${input}!=="object"||${input}===null||Array.isArray(${input}))return ${invalid}`);
        statements.push(`if(o!==D&&o.onExcessProperty==="error"&&E(${constant(emitter, ast, path)},${input},o))return ${invalid}`);
        ...
        for (let propertyIndex = 0; propertyIndex < ast.propertySignatures.length; propertyIndex++) {
          ...
          const key = propertyKey(emitter, property.name, `${propertyPath}.name`);
          const value = variable(emitter);
          const propertyStatements = [`const ${value}=${input}[${key}]`];
          ...
          statements.push(isOptional(property.type) ? `if(${propertyPresence(input, key, property.name)}){${propertyStatements.join(";")}}` : `${propertyNeedsPresenceCheck(property.name, property.type) ? `if(!(${propertyPresence(input, key, property.name)}))return ${invalid};` : ""}${propertyStatements.join(";")}`);
        }
        if (ast.indexSignatures.length > 0) emitIndexes(ast, input, output, statements, emitter, operation, path);
        return output ?? input;
      }
    case "Union":
      {
        ...
        statements.push(`const ${candidates}=U(${input},${types})`, `let ${output}=${invalid},${candidate},${decoder}`);
        if (ast.options?.mode !== "oneOf") {
          statements.push(`for(let ${index}=0;${index}<${candidates}.length;${index}++){${decoder}=${decoders}.get(${candidates}[${index}]);${candidate}=${decoder}(${input},o);if(${candidate}!==${invalid}){${output}=${candidate};break}}`);
          statements.push(`if(${output}===${invalid})return ${invalid}`);
        } else {
          const successes = variable(emitter);
          statements.push(`let ${successes}=0`);
          statements.push(`for(let ${index}=0;${index}<${candidates}.length;${index}++){${decoder}=${decoders}.get(${candidates}[${index}]);${candidate}=${decoder}(${input},o);if(${candidate}!==${invalid}){if(++${successes}>1)return ${invalid};${output}=${candidate}}}`);
...
/** @internal */
export const runtimeReference = name => `R.${name}`;
const runtimeBindings = aliases => `const {${Object.entries(aliases).map(([alias, name]) => `${name}:${alias}`).join(",")}}=R;`;
const emitOperation = (ast, operation, path = "ast") => {
  const emitter = {
    statements: [],
    helpers: [],
    initializers: [],
  ...
  const source = `"use strict";${runtimeBindings(operation === "decode" ? {
    I: "invalid",
    ...bindings
  } : bindings)}${emitter.helpers.join(";")};${emitter.initializers.join(";")};return function(i,o){${emitter.statements.join(";")};return ${output}}`;
  return {
    source,
    bindings: emitter.bindings
  };
};
const renderOperation = emitted => `(function(C,R){${emitted.source}})([${emitted.bindings.map(binding => binding.reference).join(",")}],R)`;
/** @internal */
export function generate(ast, operation) {
  if (operation === "make") {
    if (!shouldCompileMake(ast)) return undefined;
    if (ast._tag === "Objects") {
      return `if(ast.propertySignatures.some(p=>resolve(p.type).compiled!==void 0))return;return ${renderOperation(emitOperation(ast, "decode"))}`;
    }
    const element = renderOperation(emitOperation(ast.rest[0], "decode", "ast.rest[0]"));
    return `const e=resolve(ast.rest[0]),m=e.compiled===void 0?${element}:e.make;if(m===void 0)return;` + "return function(i,o){if(i===R.missing)return R.missing;if(!Array.isArray(i))return R.invalid;" + "const out=new Array(i.length);for(let x=0;x<i.length;x++){const v=m(i[x],o);" + "if(v===R.invalid||v===R.missing)return R.invalid;out[x]=v}" + "return R.getCheckIssues(ast,out,false,o)?R.invalid:out}";
  }
  ...
  const object = ast._tag === "Objects" && ast.propertySignatures.length > 0 && ast.indexSignatures.length === 0 && ast.propertySignatures.length <= maxGeneratedNodes ? emitObject(ast) : "undefined";
  const array = ast._tag === "Arrays" && ast.elements.length === 0 && ast.rest.length === 1 ? emitArray() : "undefined";
  if (operation === "makeEffect") return `return R.make(ast,resolve,${object},${array})`;
  const checkpoint = ast.encoding !== undefined && getEmission(ast, 0, true) !== "unsupported" ? `()=>${renderOperation(emitOperation(ast, "decode"))}` : "undefined";
  return `return R.decode(ast,resolve,${object},${getEmission(ast) !== "unsupported"},${checkpoint},${array})`;
}
const emitArray = () => `function({getElement,step,resume}){return function(s,input,index=0,end=input.length){
...
const inlineTypePredicate = (ast, input, path) => {
  switch (ast._tag) {
    case "Null":
      return `${input}===null`;
    case "Undefined":
...
const inlineIdentityPredicate = (ast, input, path) => ast.checks === undefined && getEncodingChecks(ast) === undefined ? inlineTypePredicate(ast, input, path) : undefined;
const emitEncoding = ast => {
  const links = ast.encoding;
  if (links?.length !== 1 || getEncodingChecks(ast) !== undefined) return undefined;
  const link = links[0];
  const transformation = link.transformation;
  if (link.to.encoding !== undefined || link.to.checks !== undefined || getEncodingChecks(link.to) !== undefined || transformation._tag !== "Transformation" || transformation.decode._tag !== "Transform") {
    return undefined;
  }
...
const emitObject = ast => {
  ...
  ast.propertySignatures.forEach((property, index) => {
    ...
    const handle = `if(r===R.sameExit){if(h${index}){${assignProperty("out", key, `v${index}`, property.name)}}}else{` + `if(!R.effectIsExit(r))return resume(state,${index},r);` + `if(r._tag==="Success"&&(value=r[R.args])!==R.missing){${assignProperty("out", key, "value", property.name)}}` + `else{t=step(state,p${index},r);if(t)return t}}`;
    ...
    if (canInlineEncoding(property.type)) {
      ...
      if (lazyProperties && links.length === 1 && property.name !== "__proto__") {
        ...
        statements.push(`const h${index}=${present},v${index}=h${index}?i[${key}]:R.missing`, `if(v${index}!==R.missing&&(${source})){const x${index}=${decoded};` + `if(x${index}!==R.missing&&(${predicate})){out[${key}]=x${index}}` + `else{t=invalid(state,${index},h${index},v${index},x${index},o);if(t)return t}}` + `else{t=fallbackProperty(state,${index},h${index},v${index},o);if(t)return t}`);
        return;
      }
      const fast = [`let x${index}=v${index};r=void 0;l${index}:{`];
      for (let linkIndex = links.length - 1; linkIndex >= 0; linkIndex--) {
        ...
        if (transformation._tag === "Transformation" && transformation.decode._tag === "Transform") {
          ...
          if (transform === undefined) {
            transform = `t${transforms.size}`;
            transforms.set(transformation.decode.transform, transform);
            initializers.push(`const ${transform}=${propertyPath}.encoding[${linkIndex}].transformation.decode.transform`);
          }
          fast.push(`x${index}=${transform}(x${index})`);
        }
        const targetPath = linkIndex === 0 ? propertyPath : `${propertyPath}.encoding[${linkIndex - 1}].to`;
        ...
        fast.push(`if(x${index}===R.missing){r=R.missingExit;break l${index}}else if(!(${predicate})){r=R.invalidEncoding(${descriptor}.type,${linkIndex},v${index},x${index},o);break l${index}}`);
      }
      fast.push(`};if(r!==void 0){${handleInline}}else{${assignProperty("out", key, `x${index}`, property.name)}}`);
      const run = `if(v${index}!==R.missing&&(${source})){${fast.join(";")}}else{r=${descriptor}.parser(v${index},o);${handleInline}}`;
      statements.push(lazyProperties ? `const h${index}=${present},v${index}=h${index}?i[${key}]:R.missing` : `const p${index}=properties[${index}],h${index}=${present},v${index}=h${index}?i[${key}]:R.missing`, run);
    } else {
      statements.push(`const p${index}=properties[${index}],h${index}=${present},v${index}=h${index}?i[${key}]:R.missing`, `r=p${index}.parser(v${index},o)`, handle);
    }
  });
  ...
  if (usesInlinePropertyFailure) {
    initializers.push("const invalid=(state,index,present,input,output,o)=>{const property=p(index);return handle(state,index,property,present,input,output===R.missing?R.missingExit:R.invalidEncoding(property.type,0,input,output,o))}", "const fallbackProperty=(state,index,present,input,o)=>{const property=p(index);return handle(state,index,property,present,input,property.parser(input,o))}");
  }
  return `function({ast,getProperties,fallback,resume,step}){${initializers.join(";")};return function(i,o){try{${statements.join(";")}}catch(e){return R.die(e)}}}`;
};
