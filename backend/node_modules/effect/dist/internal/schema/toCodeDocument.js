...
export function toCodeDocument(document) {
  ...
  function unionOptionsRuntime(options) {
      ...
      case "Arrays":
        {
          const elements = representation.elements.map((element, index) => {
            ...
            return makeCode(`${element.isOptional ? "Schema.optionalKey(" : ""}${type.runtime}${element.isOptional ? ")" : ""}${runtimeAnnotate(element.annotations, "annotateKey")}`, element.isOptional ? `(${type.Type})?` : type.Type);
          });
          ...
          if (Arr.isArrayNonEmpty(rest)) {
            ...
            if (elements.length === 0 && rest.length === 1) {
              ...
              code = makeCode(`Schema.TupleWithRest(Schema.Tuple([${elements.map(element => element.runtime).join(", ")}]), [${rest.map(item => item.runtime).join(", ")}])`, `readonly [${elements.map(element => element.Type).join(", ")}, ...Array<${item.Type}>${post.length > 0 ? `, ${post.map(item => item.Type).join(", ")}` : ""}]`);
            }
          } else {
            code = makeCode(`Schema.Tuple([${elements.map(element => element.runtime).join(", ")}])`, `readonly [${elements.map(element => element.Type).join(", ")}]`);
          }
          return code;
        }
      case "Objects":
        {
          ...
          if (indexSignatures.length === 0) {
            code = makeCode(`Schema.Struct({ ${propertyRuntimes} })`, `{ ${propertyTypes} }`);
          } else if (properties.length === 0 && indexSignatures.length === 1) {
            const signature = indexSignatures[0];
            code = makeCode(`Schema.Record(${signature.parameter.runtime}, ${signature.type.runtime})`, `{ readonly [x: ${signature.parameter.Type}]: ${signature.type.Type} }`);
          } else {
            const indexRuntimes = indexSignatures.map(signature => `Schema.Record(${signature.parameter.runtime}, ${signature.type.runtime})`).join(", ");
            const indexTypes = indexSignatures.map(signature => `readonly [x: ${signature.parameter.Type}]: ${signature.type.Type}`);
            code = properties.length === 0 ? makeCode(`Schema.StructWithRest(Schema.Struct({ ${propertyRuntimes} }), [${indexRuntimes}])`, `{ ${indexTypes.join(", ")} }`) : makeCode(`Schema.StructWithRest(Schema.Struct({ ${propertyRuntimes} }), [${indexRuntimes}])`, [`{ ${propertyTypes} }`, ...indexTypes.map(indexType => `{ ${indexType} }`)].join(" & "));
          }
          return code;
        }
      case "Union":
        {
          ...
          const options = representation.options === undefined ? "" : `, ${unionOptionsRuntime(representation.options)}`;
          return makeCode(`Schema.Union([${types.map(type => type.runtime).join(", ")}]${options})`, types.length === 0 ? "never" : types.map(type => type.Type).join(" | "));
        }
    }
  }
}
