 ...
 */
const addAvailableFront = (self, item) => {
  if (item.isAvailable || reservations.has(item)) return;
  item.isAvailable = true;
 ...
 * @since 4.0.0
 */
export const reserve = /*#__PURE__*/dual(2, (self, item) => Effect.asVoid(Effect.acquireRelease(Effect.sync(() => {
  ...
  if (!self.state.isShuttingDown && self.state.items.has(poolItem) && !self.state.invalidated.has(poolItem) && poolItem.refCount < self.config.concurrency) {
...
const invalidatePoolItem = (self, poolItem) => Effect.suspend(() => {
  ...
  return Effect.asVoid(Effect.forkIn(Effect.interruptible(resize(self)), self.state.scope, {
    startImmediately: true
  }));
});
...
const allocate = self => internal.uninterruptibleMask(restore => core.withFiber(fiber => {
  ...
  const use = Effect.flatMap(Effect.exit(impl[Acquire]), exit => {
    ...
    return Effect.as(exit._tag === "Success" ? onAcquire : Effect.flatMap(item.finalizer, () => onAcquire), item);
  });
...
const strategyUsageTTL = ttl => Effect.succeed({
  run: pool => {
  ...
  reclaim(pool) {
    return Effect.suspend(() => {
      ...
      const item = Iterable.head(Iterable.filter(pool.state.invalidated, item => !item.disableReclaim && !reservations.has(item)));
      if (item._tag === "None") {
        return Effect.undefined;
      }
