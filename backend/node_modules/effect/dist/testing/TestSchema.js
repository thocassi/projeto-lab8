...
import * as Arbitrary from "../unstable/arbitrary/Arbitrary.js";
function assertPropertyPassed(result) {
...
export class Asserts {
  ...
  static ast = {
    fields: {
      equals: (a, b) => {
        assert.deepStrictEqual(Object.fromEntries(Reflect.ownKeys(a).map(key => [key, SchemaAST.getAST(a[key])])), Object.fromEntries(Reflect.ownKeys(b).map(key => [key, SchemaAST.getAST(b[key])])));
      }
    },
    elements: {
      equals: (a, b) => {
  ...
  verifyLosslessTransformation(options) {
    ...
    return Effect.runPromise(Arbitrary.checkEffect(arbitrary, value => encodeEffect(value).pipe(Effect.flatMapEager(encoded => decodeUnknownEffect(encoded)), Effect.mapErrorEager(SchemaIssue.defaultFormatter), Effect.result, Effect.mapEager(result => isDeepStrictEqual(result, Result.succeed(value)))), options)).then(assertPropertyPassed);
  }
  /**
  ...
  arbitrary() {
    ...
    return {
      verifyGeneration(options) {
        ...
        assertPropertyPassed(Effect.runSync(Arbitrary.checkEffect(arbitrary, is, {
          runs: 20,
          ...options
        })));
      }
