 ...
 * @since 2.0.0
 */
type TupleOf_<T, N extends number, R extends Array<unknown>> = `${N}` extends `-${number}` ? never : `${N}` extends `${bigint}` ? R["length"] extends N ? R : TupleOf_<T, N, [T, ...R]> : Array<T>;
/**
...
type DeepMutableIsOpaque<T> = Extract<keyof T, symbol> extends never ? Extract<T[keyof T], Function> extends never ? false : true : true;
/**
 ...
 * @since 3.1.0
 */
export type DeepMutable<T> = T extends ReadonlyMap<infer K, infer V> ? Map<DeepMutable<K>, DeepMutable<V>> : T extends ReadonlySet<infer V> ? Set<DeepMutable<V>> : T extends ReadonlyArray<unknown> ? {
    -readonly [K in keyof T]: DeepMutable<T[K]>;
} : T extends string | number | boolean | bigint | symbol | Function ? T : DeepMutableIsOpaque<T> extends true ? T : {
    -readonly [K in keyof T]: DeepMutable<T[K]>;
};
/**
...
export type RequiredKeys<T> = RequiredKeysFrom_<[
    T
] extends [ReadonlyArray<unknown>] ? T : IsIndexKey_<keyof T> extends true ? WithoutIndexSignature_<T> : T>;
export {};
